...
export interface AreaPolygonResponse {
...
export interface TilegenHistoryEntry {
...
export interface TilegenHistoryResponse {
  entries: TilegenHistoryEntry[];
}

/** Live status of one transit tile layer being generated by tilegen. */
export interface TilegenLayerStatus {
...
export interface TilegenStatusResponse {
  layers: TilegenLayerStatus[];
}
 ...
 */
    ...
    /**
     * @description Uses PostGIS `ST_LineMerge` + `ST_LineLocatePoint` + `ST_LineSubstring` to compute the slice server-side in a single query — much faster than fetching the full geometry and slicing client-side.
     *
     * @tags routes
     * @name GetRouteSegment
     * @summary Extract a segment of a route's geometry between two points (e.g. platforms).
     * @request POST:/api/routes/segment
     */
    getRouteSegment: (data: RouteSegmentRequest, params: RequestParams = {}) =>
      this.request<RouteSegmentResponse, ErrorResponse>({
 ...
 * @tags tilegen
 * @name GetAreaPolygon
 * @summary Return the actual administrative polygon for a named area (German
 ...
 * @request GET:/api/tilegen/area-polygon
 */
    getAreaPolygon: (
      query: {
      ...
      this.request<AreaPolygonResponse, ErrorResponse>({
     ...
     * @tags tilegen
     * @name GetHistory
     * @summary Get past tile generation runs (most recent first).
     * @request GET:/api/tilegen/history
     */
    getHistory: (
      query?: {
      ...
      this.request<TilegenHistoryResponse, ErrorResponse>({
     ...
     * @tags tilegen
     * @name GetStatus
     * @summary Get live tile generation status for the diagnostics panel.
     * @request GET:/api/tilegen/status
     */
    getStatus: (params: RequestParams = {}) =>
      this.request<TilegenStatusResponse, ErrorResponse>({
