            ...
            CaptionLabel: ({
              ...
              return (
                <span className={captionClassName}>
                  ...
                  <button
                    type={`button`}
                    onPointerDown={(e) => { if (e.button === 0) { e.preventDefault(); handleYearClick() } }}
        ...
        <div className={`bg-popover absolute inset-0 z-10 flex flex-col items-center rounded-md p-3`}>
          <div className={`mb-2 flex h-(--cell-size) w-full items-center justify-between`}>
            <Button
              ...
              variant={`ghost`}
              size={`icon`}
              onPointerDown={(e) => { if (e.button === 0) { e.preventDefault(); setDecadeStart((d) => d - YEARS_PER_PAGE) } }}
            ...
            {years.map((year) => {
              ...
              return (
                <Button
                  ...
                  size={`sm`}
                  onPointerDown={(e) => { if (e.button === 0) { e.preventDefault(); handleYearSelect(year) } }}
...
const CalendarDayButton = ({
  ...
  // react-day-picker calls setFocused() on pointerdown which triggers a
  // re-render that can replace the DOM node before the browser fires the
  // native "click" event. Move the onClick handler to onPointerDown so the
...
/** Nav button that fires onClick on pointerdown to avoid the same re-render
 *  race that affects day buttons (see CalendarDayButton comment). */
